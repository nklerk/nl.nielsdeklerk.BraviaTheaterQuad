...
const MIN_BASS_LEVEL_NO_SUB = 0;
const MAX_BASS_LEVEL_NO_SUB = 2;
...
class BraviaClient extends EventEmitter {
  ...
  async detectSubwoofer() {
    const currentLevel = await this.getBassLevel();

    // If already outside 0-2 range, definitely has subwoofer
    if (currentLevel < MIN_BASS_LEVEL_NO_SUB || currentLevel > MAX_BASS_LEVEL_NO_SUB) {
      this.log(`Subwoofer detected: bass level ${currentLevel} outside 0-2 range`);
      return true;
    }
    ...
    try {
      ...
      if (this._isAck(resp)) {
        this.log('Subwoofer detected: device accepted bass level -1');
    ...
    this.log('No subwoofer detected');
    return false;
  }
